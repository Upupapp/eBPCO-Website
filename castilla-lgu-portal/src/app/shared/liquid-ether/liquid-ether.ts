...
// Angular port of the LiquidEther WebGL fluid-simulation background.
// The simulation internals (shaders, FBO pipeline, mouse/auto-drive logic)
// are unchanged from the original React implementation — only the outer
// lifecycle wiring (useRef/useEffect -> ElementRef/ngAfterViewInit/OnDestroy)
// was adapted for Angular.
@Component({
  selector: 'app-liquid-ether',
  imports: [],
  templateUrl: './liquid-ether.html',
  styleUrl: './liquid-ether.scss',
})
export class LiquidEther implements AfterViewInit, OnChanges, OnDestroy {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (!this.initialized || !this.webgl) return;

    // Color-palette changes require a full teardown/rebuild since the
    // palette texture is baked once at init time.
  ...
  private setup(): void {
    ...
    // WebGLManager's own methods run with `this` bound to the WebGLManager
    // instance, not this Angular component — capture the component here so
    // those methods can still read/write its rafId/isVisible fields.
    const self = this;
