...
import 'steps/step1_related_building_permit.dart';
import 'steps/step2_applicant_info.dart';
import 'steps/step3_project_information.dart';
import 'steps/step4_design_professional.dart';
import 'steps/step5_required_documents.dart';
import 'steps/step6_review_declaration.dart';
...
/// FSEC for Building Permit (BFP) application wizard — a 6-step flow
/// completely separate from every other permit in this app: its own
/// model ([FsecPermitDraft]), its own provider ([FsecPermitProvider]),
/// and its own step widgets. Unlike the Sign/Fencing Permits' gated
/// "related Building Permit" step, Step 1 here is purely informational —
/// an FSEC is typically filed and evaluated BEFORE the Building Permit
/// it supports has been approved, since it is one of the inputs required
/// for that approval. Structurally mirrors every other wizard (progress
/// header, PageView, bottom action bar) so all permits feel identical to
/// use.
class FsecPermitWizardScreen extends StatefulWidget {
  const FsecPermitWizardScreen({super.key});

  @override
  State<FsecPermitWizardScreen> createState() =>
      _FsecPermitWizardScreenState();
}

class _FsecPermitWizardScreenState extends State<FsecPermitWizardScreen> {
  ...
  static const List<_StepMeta> _stepMeta = [
    _StepMeta(
      ...
      subtitle: 'Optionally reference the Building Permit this supports.',
    ),
    _StepMeta(
      title: 'Applicant Information',
      subtitle: 'Tell us who is applying for the FSEC.',
    ),
    _StepMeta(
      title: 'Project Information',
      subtitle: 'Describe the project and its fire-safety scope.',
    ),
    _StepMeta(
      title: 'Design Professional',
      subtitle: 'Provide the architect or engineer of record.',
    ),
    _StepMeta(
  ...
  late FsecPermitDraft _draft;
  late int _currentStep;

  @override
  void initState() {
    super.initState();
    final provider = context.read<FsecPermitProvider>();
  ...
  void _goToStep(int step) {
    ...
    context.read<FsecPermitProvider>().goToStep(step);
  ...
  void _handleSubmit() {
    ...
    context.pushReplacement(
      '/applications/new/fsec-permit/submitted',
      extra: {
        'referenceNumber': referenceNumber,
        'submissionDate': now,
        'relatedBuildingPermitApplicationNumber':
            _draft.relatedBuildingPermit.buildingPermitApplicationNumber,
      },
    );
  }

  void _handleSaveDraft() {
    context.read<FsecPermitProvider>().saveAsDraft();
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('FSEC for Building Permit (BFP)'),
          leading: IconButton(
              ...
              Expanded(
                child: PageView(
                  ...
                  children: [
                    ...
                    Step4DesignProfessional(
                      formKey: _formKeys[3],
                      draft: _draft,
                      onChanged: _onDraftChanged,
                    ),
                    Step5RequiredDocuments(
                      formKey: _formKeys[4],
                      draft: _draft,
                      onChanged: _onDraftChanged,
                    ),
                    Step6ReviewDeclaration(
                      formKey: _formKeys[5],
                      draft: _draft,
                      onChanged: _onDraftChanged,
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text(
            'Complete your FSEC application step by step.',
