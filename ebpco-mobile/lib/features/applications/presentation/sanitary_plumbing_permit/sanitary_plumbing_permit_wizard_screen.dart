...
import 'steps/step3_scope_occupancy.dart';
...
/// Sanitary / Plumbing Permit application wizard — a full 9-step flow
/// based on the official Sanitary / Plumbing Permit form, completely
/// separate from New Construction, Renovation, Addition/Extension,
/// Demolition, Architectural, Civil/Structural, Electrical, and
/// Mechanical: its own model ([SanitaryPermitDraft]), its own provider
/// ([SanitaryPlumbingPermitProvider]), and its own step widgets.
/// Structurally mirrors the other eight wizards (progress header,
/// PageView, bottom action bar) so all nine feel identical to use.
class SanitaryPlumbingPermitWizardScreen extends StatefulWidget {
  const SanitaryPlumbingPermitWizardScreen({super.key});

  @override
  State<SanitaryPlumbingPermitWizardScreen> createState() =>
      _SanitaryPlumbingPermitWizardScreenState();
}

class _SanitaryPlumbingPermitWizardScreenState
    extends State<SanitaryPlumbingPermitWizardScreen> {
  ...
  static const List<_StepMeta> _stepMeta = [
    _StepMeta(
      ...
      subtitle:
          'Provide the details of the person applying for the Sanitary / Plumbing Permit.',
    ),
    _StepMeta(
    ...
    _StepMeta(
      ...
      subtitle:
          'Provide fixture quantities, water-supply details, disposal systems, building measurements, schedule, and cost.',
    ),
    _StepMeta(
      title: 'Sanitary / Plumbing Professionals',
      subtitle:
    ...
    _StepMeta(
      ...
      subtitle:
          'Upload the sanitary and plumbing documents required for evaluation.',
    ),
    _StepMeta(
      title: 'Review & Declaration',
      subtitle:
          'Review the Sanitary / Plumbing Permit application before submission.',
    ),
    _StepMeta(
      title: 'Evaluation & Permit Status',
      subtitle: 'Track document review, processing, and permit action.',
    ),
  ];
  ...
  late SanitaryPermitDraft _draft;
  late int _currentStep;

  @override
  void initState() {
    super.initState();
    final provider = context.read<SanitaryPlumbingPermitProvider>();
  ...
  void _goToStep(int step) {
    ...
    context.read<SanitaryPlumbingPermitProvider>().goToStep(step);
  ...
  void _handleSubmit() {
    ...
    final referenceNumber =
        'SAN-${now.year}-${(now.millisecondsSinceEpoch % 900000 + 100000)}';
  ...
  void _handleSaveDraft() {
    context.read<SanitaryPlumbingPermitProvider>().saveAsDraft();
